...
public class JsonSerializationVisitor implements Visitor {
	...
	/**
	 * If true then the names of the elements should be suppressed because we are in an array.
	 */
	 ...
	 *
	 * @param sink The Writer to write to.
	 * @param oneLine If true then the visitor will write the document to 1 line, otherwise the visitor will write the document accross multiple lines with indenting.
	 */
	...
	public void visit(final List<Element> elements) {
		try {
			if (elements.isEmpty()) {
				mySink.write("{}");
			} else if ((elements.size() == 1) && !(elements.get(0) instanceof DocumentElement) && !(elements.get(0) instanceof ArrayElement)) {
				mySink.write("{ ");
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Overridden to append a JSON representation of the binary element to the writer provided when this object was created. This method generates the MongoDB standard BinData(...)
	 * JSON extension.
	 * </p>
	 */
	@Override
	public void visitBinary(final String name, final byte subType, final byte[] data) {
		try {
			writeName(name);
			mySink.write('"');
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Overridden to append a JSON representation of the DBPointer element to the writer provided when this object was created. This method generates the non-standard
	 * DBPointer(...) JSON extension.
	 * </p>
	 */
	@Override
	public void visitDBPointer(final String name, final String databaseName, final String collectionName, final ObjectId id) {
		try {
			writeName(name);
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Overridden to append a JSON representation of the JavaScript element to the writer provided when this object was created. This method writes the elements as a
	 * <code>{ $code : &lt;code&gt; }</code> sub-document.
	 * </p>
	 */
	@Override
	 ...
	 * <p>
	 * Overridden to append a JSON representation of the JavaScript element to the writer provided when this object was created. This method writes the elements as a
	 * <code>{ $code : &lt;code&gt;, $scope : &lt;scope&gt; }</code> sub-document.
	 * </p>
	 */
	@Override
	public void visitJavaScript(final String name, final String code, final Document scope) {
		try {
			writeName(name);
			mySink.write("{ $code : ");
			writeQuotedString(code);
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Overridden to append a JSON representation of the binary element to the writer provided when this object was created. This method generates the MongoDB standard
	 * NumberLong(...) JSON extension.
	 * </p>
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Overridden to append a JSON representation of the DBPointer element to the writer provided when this object was created. This method generates the non-standard MaxKey() JSON
	 * extension.
	 * </p>
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Overridden to append a JSON representation of the DBPointer element to the writer provided when this object was created. This method generates the non-standard MinKey() JSON
	 * extension.
	 * </p>
	 */
	@Override
	 ...
	 * <p>
	 * Overridden to append a JSON representation of the JavaScript element to the writer provided when this object was created. This method writes the elements as a
	 * <code>{ $regex : &lt;pattern&gt;, $options : &lt;options&gt; }</code> sub-document.
	 * </p>
	 */
	@Override
	public void visitRegularExpression(final String name, final String pattern, final String options) {
		try {
			writeName(name);
			mySink.write("{ $regex : '");
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Overridden to append a JSON representation of the binary element to the writer provided when this object was created. This method generates the MongoDB standard ISODate(...)
	 * JSON extension.
	 * </p>
	 */
	@Override
	...
	/**
	 * Returns if the visitor is currently suppressing the names of elements. This is true when serializing an array.
	 *
	 * @return If the visitor is currently suppressing the names of elements. This is true when serializing an array.
	 */
	...
	/**
	 * Writes a new line if {@link #myOneLine} is false and indents to the {@link #myIndentLevel}.
	 *
	 * @throws IOException On a failure to write the new line.
	 */
	...
	/**
	 * Sets the value for if the visitor is currently suppressing the names of elements. This is true, for example, when serializing an array.
	 *
	 * @param suppressNames The new value for if names should be suppressed.
	 */
	 ...
	 *
	 * @param name The name to write, if not suppressed.
	 * @throws IOException On a failure to write the new line.
	 */
	 ...
	 *
	 * @param id The {@link ObjectId} to write.
	 * @throws IOException On a failure writing to the sink.
	 */
	 ...
	 * @throws IOException On a failure writing the String.
	 */
	protected void writeQuotedString(final String string) throws IOException {
		if (string.indexOf('"') < 0) {
			mySink.write('"');
			mySink.write(escapeNewlines(string));
			mySink.write('"');
		} else {
			mySink.write('"');
			// Escape any embedded double quotes.
			mySink.write(escapeNewlines(string).replaceAll("\"", "\\\""));
			mySink.write('"');
		}
	}

	protected String escapeNewlines(final String string) {
		return string.indexOf("\n") == -1 && string.indexOf("\r") == -1 ? string : string.replaceAll("\n|\r", "\\\\n");
	}
}
