...
public class FeaturedPresentationsController extends AbstractController {
	...
	public List<PresentationCategory> completeCategory(final String query) {
		...
		return Arrays.stream(PresentationCategory.values()).filter(availableCategoriesPredicate).filter(queryPredicate).collect(Collectors.<PresentationCategory>toList());
	}
